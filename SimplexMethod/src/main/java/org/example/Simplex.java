...
public class Simplex {
    ...
    private final int cols; // row and column
    private final float[][] table; // simplex tableau
    ...
    // should be used in a loop to continously compute until
    ...
    private int findEnteringColumn(){
        ...
        for(pos = 0; pos < cols-1; pos++){
            if(table[rows-1][pos] < 0){
                count++;
                location = pos; // Update the location whenever a negative value is found
            }
        }
        ...
        if(count > 1){
            ...
            location = -1; // No negative values found, set to an invalid index
        }
    ...
    private int findSmallestValue(float[] data) {
        float minimum = Float.POSITIVE_INFINITY; // Initialize to positive infinity
        int location = -1; // Initialize to an invalid index

        for (int c = 0; c < data.length; c++) {
            if (data[c] > 0 && data[c] <= minimum) {
                if (data[c] < minimum || c < location) {
                    minimum = data[c];
                    location = c;
                }
            }
    ...
    private int findLargestValue(float[] data) {
        float maximum = Float.NEGATIVE_INFINITY; // Initialize to negative infinity
        int location = -1; // Initialize to an invalid index

        for (int c = 0; c < data.length; c++) {
            if (Float.compare(data[c], maximum) >= 0) {
                if (Float.compare(data[c], maximum) > 0 || c < location) {
                    maximum = data[c];
                    location = c;
                }
            }
